...
### 🗣️ Giọng đọc AI miễn phí của Microsoft (job `marketing`)

Không cần tự thu âm hay upload file audio — chỉ cần điền **nội dung cần đọc**, GitHub sẽ tự tạo giọng đọc bằng Microsoft edge-tts (miễn phí, chất lượng cao).

> Lưu ý: tính năng này **chỉ chạy được trên GitHub Actions**, không chạy trên Hugging Face free (HF chặn mạng tới server TTS của Microsoft). Đây là một lợi thế của bản GitHub.

Trong job `marketing`, thay vì `voice` (link audio), dùng:
...
```json
{
  ...
  "output": "video.mp4",
  "resolution": "tiktok",
  "images": ["https://.../anh1.jpg"],
  "voice_text": "Be still and know that I am God...",
  "voice_name": "aria",
  "voice_rate": "-8%"
}
```

- `voice_text`: nội dung tiếng Anh cần đọc.
- `voice_name`: chọn 1 tên bên dưới.
- `voice_rate`: tốc độ, ví dụ `"-12%"` (chậm) → `"+4%"` (nhanh). Mặc định `"-8%"`.

**Giọng NỮ (hợp POH):**

| Tên | Giọng | Ghi chú |
|-----|-------|---------|
| `aria` | en-US-AriaNeural | Ấm, thân thiện, tự nhiên — **khuyên dùng** |
| `jenny` | en-US-JennyNeural | Dịu dàng, kể chuyện |
| `michelle` | en-US-MichelleNeural | Trưởng thành, điềm tĩnh |
| `sonia` | en-GB-SoniaNeural | Anh-Anh, thanh lịch |

**Giọng NAM (hợp POH):**

| Tên | Giọng | Ghi chú |
|-----|-------|---------|
| `guy` | en-US-GuyNeural | Trầm ấm, đáng tin — **khuyên dùng** |
| `davis` | en-US-DavisNeural | Điềm đạm, nghiêm túc |
| `tony` | en-US-TonyNeural | Năng lượng, tươi |
| `roger` | en-US-RogerNeural | Trung tính, rõ ràng |
| `ryan` | en-GB-RyanNeural | Anh-Anh, ấm |

> Nếu vẫn muốn dùng file audio thu sẵn, cứ để trường `voice` (link) như cũ — bỏ `voice_text` đi. Nếu có cả hai, `voice_text` được ưu tiên.
